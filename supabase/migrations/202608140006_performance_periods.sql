...
update public.performance_metrics set period_start = metric_date where period_start is null;
update public.performance_metrics set period_end = metric_date where period_end is null;
alter table public.performance_metrics alter column period_start set not null;
alter table public.performance_metrics alter column period_end set not null;
alter table public.performance_metrics drop constraint if exists performance_metrics_user_id_experiment_id_metric_date_source_key;
create unique index if not exists performance_metrics_period_uidx on public.performance_metrics(user_id, experiment_id, period_start, period_end, source);
alter table public.performance_metrics drop constraint if exists performance_metrics_period_order_check;
alter table public.performance_metrics add constraint performance_metrics_period_order_check check (period_start <= period_end);

drop function if exists public.save_performance_decision(uuid,date,bigint,bigint,bigint,bigint,numeric,numeric,text,text,text,numeric,jsonb,uuid,uuid);
...
begin
  ...
  values (v_user_id, 'PERFORMANCE_RECORDED', 'content_experiment', p_experiment_id::text,
    jsonb_build_object('decision_id', v_decision_id, 'metric_id', v_metric_id, 'period_start', p_period_start, 'period_end', p_period_end, 'state', p_state));
